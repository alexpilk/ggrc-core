...
def reindex(task):
  ...
  if objects_to_reindex:
    indexed_models = collect_selected_models(objects_to_reindex)
  else:
    indexed_models = collect_all_models()
...
def collect_all_models():
  ...
  return {
      ...
      if can_be_reindexed(model)
  }


def collect_selected_models(objects_to_reindex):
  ...
  if unsupported_models:
    raise exceptions.BadRequest(
        'Models "{}" either do not exist or cannot be reindexed'.format(
...
def admin_reindex():
  """Calls a webhook that reindexes indexable objects
  """
  objects = flask.request.get_json().get("objects", []) \
      if flask.request.data else []
